...
| 2026-08-05 | 本番用xlsm(`在庫管理システム_v1.0_本番用.xlsm`)は復元しない・保留 | 復元すると最新版がどちらか混乱する | 即時復元 / 削除確定 | git履歴にバックアップ確保済みで急ぐ理由がない | 在庫管理Excel運用。**当初「Phase1.5終了まで」の保留だったが、Phase番号統一によりその期限は経過。扱いの再判断が必要(§未確定事項)** |
...
| 本番用xlsmの扱い | ⬜ **再判断が必要。** 当初「Phase1.5終了まで保留」だったが、Phase番号統一でPhase1.5がPhase1に統合され期限が経過。復元 / 削除確定 / 保留継続 のいずれかをユーザーが決定する(それまでClaudeは一切操作しない) |
