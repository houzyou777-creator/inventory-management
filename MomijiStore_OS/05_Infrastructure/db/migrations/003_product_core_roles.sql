...
-- 事実の写し・コード・観測値
GRANT INSERT ON product_core.identifier, product_core.cost_observation TO pc_ingest;
GRANT INSERT (listing_group_id, channel, group_type, group_key, source_type, source_ref, created_by, ingest_run_id)
...
GRANT INSERT (listing_id, channel, asin, seller_sku, rakuten_item_id, rakuten_sku, fulfillment, listing_group_id,
              legacy_listing_id, channel_status, first_seen_at, last_seen_at, source_type, source_ref, created_by,
              ingest_run_id)
...
GRANT INSERT (pp_id, name, kind, origin_key, note, created_by, ingest_run_id) ON product_core.physical_product TO pc_ingest;
GRANT UPDATE (name, kind, note, updated_by) ON product_core.physical_product TO pc_ingest;
GRANT INSERT (cp_id, name, comp_type, origin_key, note, created_by, ingest_run_id) ON product_core.composition TO pc_ingest;
GRANT UPDATE (name, comp_type, note, updated_by) ON product_core.composition TO pc_ingest;
-- 構成品の下書き(親が PROVISIONAL の間だけ。トリガーで確認)
GRANT INSERT, DELETE ON product_core.composition_component TO pc_ingest;
GRANT UPDATE (pp_id, quantity, role, sort_no, evidence) ON product_core.composition_component TO pc_ingest;

-- AI 判定(review 欄には書けない)
GRANT INSERT (assessment_id, assessment_type, subject_entity_id, subject_entity_type, subject_key, verdict,
...
GRANT INSERT (link_id, identifier_id, entity_id, entity_type, link_basis, scan_policy, valid_from, valid_to, confidence,
              evidence, assessment_id, created_by, ingest_run_id)
    ON product_core.identifier_link TO pc_ingest;
GRANT INSERT (mapping_id, legacy_p, entity_id, entity_type, mapping_role, valid_from, valid_to, legacy_snapshot_ref,
              assessment_id, created_by, ingest_run_id)
    ON product_core.legacy_mapping TO pc_ingest;
GRANT INSERT (relationship_id, rel_type, from_pp_id, to_pp_id, variant_group_key, variant_axis, evidence, assessment_id,
              created_by)
    ON product_core.product_relationship TO pc_ingest;
GRANT INSERT (ref_id, listing_id, entity_id, entity_type, valid_from, valid_to, derived_from_legacy_p, reason,
...
GRANT INSERT ON product_core.identifier_link, product_core.legacy_mapping, product_core.product_relationship,
                product_core.listing_reference_history TO pc_reviewer;
