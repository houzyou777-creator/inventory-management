...
# ──────────────────────────────────────────────────────────────
TASKS = [
    # ── V-2(取込4列の文字列書式)。コピー検証 PASS・本番差し替え PASS(2026-09-12 22:37) ──
    ('V2-1〜9', 'V-2 コピー検証', f'{V2}/…xlsm', '—', '(完了)', '✅ verify_v2_import.py PASS(2026-09-12 22:30)', '済'),
    ('V2P-1〜6', 'V-2 本番モジュール差し替え', 'SourceData/楽天在庫金額集計ツール_v1.0.xlsm', '—', '(完了)', '✅ VBA一致・全セル不変・ブック正常(2026-09-12 22:37)', '済'),
    # ── V-3(②集計先も文字列書式 ＋ ③識別子の型統一)。まずコピーで ──
    #    2026-09-15 07:44 の1回目はほぼPASS。空欄が長さ0の文字列になる点だけ直したので、V3-2〜V3-6 をもう一度
    ('V3-1', 'V-3 コピー検証(2回目)', f'{V2}/楽天在庫金額集計ツール_v1.0.xlsm(baselineの**外**)', 'Excelで開く', 'マクロを有効にして開く(1回目の結果は検証済み。空欄の扱いを直した V3 を入れ直す)', 'シートが見える', ''),
    ('V3-2', '', '同上', 'VBE', 'option＋F11 → 左で Module_Rakuten_Tool を選択 → メニューバー ファイル → 「Module_Rakuten_Tool の削除...」→ エクスポートは「いいえ」', '一覧から消える', ''),
    ('V3-3', '', '同上', 'VBE', 'ファイル → ファイルのインポート... → ' + f'{V2}/Module_Rakuten_Tool_V3.bas', 'Module_Rakuten_Tool が1つだけ戻る → VBEを閉じる', ''),
    ('V3-4', '', '同上', 'ボタン', '「楽天在庫ファイル読込」→ 上書き「はい」→(アクセス許可が出たら「アクセス権を付与」)→ 完了「OK」', '完了メッセージに「識別子の型変換: N件」が出る。K列 読込日時が今', ''),
    ('V3-5', '', '同上', 'ボタン', '「集計実行」→ OK', '在庫金額集計の集計日時が今', ''),
    ('V3-6', '', '同上', '保存', '⌘S → 閉じる(**コピー検証の最終操作**)', 'Claude Code が verify_v2_import.py で判定', ''),
    # ── V-3 本番反映(コピーPASS後。Claude Code が「V3P 着手OK」と伝えてから) ──
    ('V3P-1', 'V-3 本番反映(PASS後)', 'SourceData/楽天在庫金額集計ツール_v1.0.xlsm(**本番**)', 'Excelで開く', 'マクロ有効。**読込・集計は押さない**', 'シートが見える', ''),
    ('V3P-2', '', '同上', 'VBE', 'Module_Rakuten_Tool を選択 → ファイル → 削除... → いいえ → ファイル → インポート → Module_Rakuten_Tool_V3.bas → VBEを閉じる', 'Module_Rakuten_Tool が1つだけ', ''),
    ('V3P-3', '', '同上', '保存', '⌘S → 閉じる(**最終操作**)', 'Claude Code がデータ不変・VBA一致・ブック正常を検証', ''),
]
