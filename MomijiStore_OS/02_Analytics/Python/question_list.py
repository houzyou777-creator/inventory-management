...
V4 = 'MomijiStore_OS/01_InventoryManagement/SourceData/V4_test'
...
TASKS = [
    ...
    # 2026-09-21: T-10(ファイル整理2回目)は英樹がチャットで承認(Q18 実行してよい／Q19 削除候補Archiveへ退避)→ 実施済み。いま英樹がやる操作は無い
]
DONE = [
    ...
    ('T-3d-1/Q14', '楽天CSVに無い94件の扱い', 'ChatGPT判断(2026-09-16)「最新CSVに未掲載・現在在庫未確認として別枠管理」。コピーの Import を別枠管理版で作成(本体814行／未掲載_未確認 94行・旧数量469・参考額1,286,025)。英樹の回答は「消す」→ 食い違いを ChatGPTへの確認 に記載(集計値は同じ)'),
    ('T-3d-2', 'コピーで取込→集計(英樹 2026-09-16 13:08)', '実測: 総在庫数量 5,911／総在庫金額 12,621,422／原価未登録 59／共有(-a) 367行 2,536個 5,454,250 → Python予測と**完全一致**。識別子 814行すべて文字列・不一致0・空欄0・識別子チェック(L列)0。VBAは本番と同一。b087lzdd59=5036/1273、b0dktchdgk=…659-2(未登録)'),
    ('T-3d-3/Q15', 'b0dktchdgk の商品番号', '英樹回答「4987176260659-2(CSVのまま)」。V-3 で本番 楽天CSV取込 行583 を …635-2 にした訂正は誤り(取り下げ記録 Output/商品番号_訂正取り下げ_20260916.csv)。本番反映(再取込)で …659-2 に置き換わる。反映しない場合は行583を戻す作業が別に必要'),
    ('T-3e/Q16', '本番反映の判断', 'ChatGPT判断(2026-09-16): V-4 検証PASS＋本番VBA差し替え後のデータ不変を確認できたら 9/16資料の本番反映へ進めてよい(再承認不要)。本番ツール・楽天Import・Amazonリストをバックアップし対象フルパスを確認してから着手OK'),
    ('T-3f/Q14・Q17', 'ChatGPTの判断', 'Q14: 英樹の回答「消す」を採用(94件を運用中の一覧から除外。旧記録はバックアップ＋変更ログ。在庫0・損失確定の意味ではない。Amazon40件へは広げない)／Q17: V-4 承認(本番反映より先に実施。条件6項目)'),
    ...
    ('T-10/Q18・Q19', 'ファイル整理(2回目)', '英樹 2026-09-21 チャットで承認: Q18 移動38件「実行してよい」／Q19 削除候補31件「削除候補Archiveへ退避(物理削除はしない)」。実行前に固定参照を検索(運用スクリプトは本番パスのみ参照・検証用モードは要コピー再作成)→ `tidy_files.py apply-quarantine --round2` で退避38＋隔離32(削除0)。対応表 Output/Archive/移動対応表_20260920.csv'),
    ('T-9', '8月の最終確認(16行)', '英樹 2026-09-21 回答 16/16(保留0)。ChatGPT承認後に本番反映(2026-09-20 23:07): F-01 IROKA 7,164→7,536／F-02 2,642→2,862／F-03 1,152→1,232、F-04〜12 現在値のまま英樹確認済み(一次根拠未照合)、F-13/14 確認済み0円、F-15/16 共有→例外一覧で承認・楽天単独から除外(445SKU・¥7,024,963)。Amazon チャネル実額手数料 ¥759,756 を Q:R に別管理。検証PASS'),
...
KNOWN = [
    ...
    ('2026-09-21', 'ファイル整理2回目: Q18 移動38件=実行してよい／Q19 削除候補31件=削除候補Archiveへ退避(物理削除はしない。「削除承認」ではなく「隔離・退避」)'),
]
...
def build_questions():
    ...
    Q.append(('Q16', APPROVE,
              '【承認・本番反映】コピーでの取込・集計(9/16 13:08)は Python予測と完全一致しました: 総在庫数量 5,911／総在庫金額 12,621,422／原価未登録 59件(154個)／共有在庫(-a) 367行 2,536個 5,454,250／共有除外後 3,375個 7,167,172。'
              '8/3資料(6,801個・15,077,081)との差は「今回のCSVに無い94件(旧469個・参考額1,286,025)」と在庫数の増減(287行)によるものです。'
              '未確認のまま残るもの: ①94件の現在在庫 ②原価未登録59件(うち-a 15件) ③Amazon在庫の基準日時(取得08:15のみ) ④集計後にファイルが 300KB→7MB になる(Q17)。'
              '本番の 楽天在庫金額集計ツール と Import(楽天/Amazon在庫リスト)へ、この最新CSV(9/16 08:11)の内容を反映してよいですか？ '
              '(実施する場合: Claude Code が本番ツール・本番Import・Amazonリストをバックアップ → Import を差し替え → 英樹が本番で「読込」→「集計」→保存。着手OKは別途出します)',
              '本番 楽天在庫金額集計ツール_v1.0.xlsm／Import/楽天在庫リスト_import.xlsx／Amazon在庫リスト_import.xlsx', '実施する／しない／ChatGPTに確認する', '本番反映(最新CSV 2026-09-16 08:11)'))
    Q.append(('Q17', APPROVE,
              '【承認・ツール修正 V-4】V-3 の副作用が見つかりました: 集計実行のたびに「在庫金額集計」と「要確認一覧」の10万行分に文字列書式を付けるため、保存後のファイルが 300KB → 約7MB になります(コピーで確認。V-3 のテストコピーも 7MB でした)。'
              '数値・集計結果に誤りはありません。書式を付ける範囲を「書き込んだ行だけ」に直す修正(V-4)を、いつもの手順(コピーで検証 → ChatGPTレビュー → 本番差し替え)で進めてよいですか？',
              'VBA Module_Rakuten_Tool(在庫金額集計 A10:L100000・要確認一覧 A3:H100000 の NumberFormat)', 'コピーで検証してよい／ChatGPTに確認する／しない', 'V-4 ファイル肥大の修正'))
    Q.append(('Q18', APPROVE,
              '【承認・ファイル整理(2回目)】8月締めで検証用フォルダやバックアップが増えたので整理します。案は Output/整理案_20260921.txt。'
              '「移動(退避)38件」は Archive/2026-09/{確認リスト,報告,証跡,生成物,検証}/ や Backup/Archive/ へ動かすだけ(名前は変えず、消しません。'
              '検証フォルダ V2_test/V3_stock_test/V4_test/Summary_test は丸ごと退避)。実行してよいですか？',
              'Output/整理案_20260921.txt の「移動(退避)候補」', '実行してよい／一部だけ(補足に書く)／まだ', 'ファイル整理2回目(移動)'))
    Q.append(('Q19', APPROVE,
              '【承認・削除候補(2回目)】同じ整理案の「削除候補31件・2.2MB」(確認リストの再生成前の一時バックアップ14本、在庫サマリー検証の「複製の複製」17本)を消してよいですか？ '
              '前回(Q13)と同じく、承認後も Claude Code は消さずに Archive/削除候補_20260921/ へ退避し、対応表に記録します(実際に消すのは英樹の判断のとき)。',
              'Output/整理案_20260921.txt の「削除候補」', '全部消してよい(退避)／一部だけ(補足に書く)／消さない', 'ファイル整理2回目(削除候補)'))
...
CHATGPT_DECISIONS = {
    ...
    'Q14': 'ChatGPT判断(2026-09-16 2回目): 英樹の明示回答「消す(RMSに合わせる)」を採用。楽天94件を最新在庫一覧から除外し、旧記録はバックアップと変更ログ(楽天在庫リスト_import_削除記録_*.csv)に保存。'
           '運用中の一覧に「未掲載_未確認」シートは必須にしない。削除を在庫0・損失確定と解釈しない。販売実績・商品マスターは削除しない。Amazon40件など別対象へ自動的に広げない'
           '(1回目の判断「別枠管理」は英樹の回答で置き換え)',
    'Q16': 'ChatGPT判断(2026-09-16): V-4 の検証が通り、本番VBA差し替え後のデータ不変が確認できたら 9/16資料の本番反映へ進めてよい(条件を満たした完了報告に再承認は不要)。'
           '本番ツール・楽天Import・Amazonリストをバックアップし対象フルパスを確認してから英樹へ着手OK。検証時から資料・本番の状態が変わっていれば差を確認。反映後はコピー実測との一致と未評価件数を報告。'
           '楽天 5,911・12,621,422円は共有在庫込みの掲載分集計、共有除外後 3,375・7,167,172円。全社の確定在庫数量・確定評価額とは表示しない。資料の基準日時と取込実行日時を分けて記録',
    'Q17': 'ChatGPT判断(2026-09-16): V-4 の修正・コピー検証を承認。本番の最新在庫反映より先に実施。条件: 識別子の文字列書式は代入前にそのセルへ／10万行一律をやめる／旧書式の整理は対象を限定し見出し・金額・日付・数式を壊さない／'
           '保存・閉じる・再読込後のサイズ確認／識別子完全一致・空欄保持・集計値・数式・ボタン・VBAの検証／2回目の取込・集計でも肥大せず、行数が減っても旧データが残らない',
}
...
CHATGPT_NOTES = {
    ...
    'Q14': '94件は「今回のCSV(9/16 08:11)に無い」だけで、RMSから消えたかは未確認。旧数量469・旧単価での参考額1,286,025円。'
           ...
           '【食い違い】英樹の回答は「消す(RMSに合わせる)」。ツールの集計値(5,911/12,621,422)は「消す」でも「別枠管理」でも同じ(本体は最新掲載分だけのため)。'
           '違いは94件の旧数量・旧単価の記録を別シートに残すか捨てるかだけ。Claude Code はどちらも実行せず、判断を待つ(現状は別枠管理のまま)。',
    'Q17': 'V-3 で 在庫金額集計 A10:L100000 / 要確認一覧 A3:H100000 の識別子列に NumberFormat="@" を付けている。Excel は書式付きの空セルも保存するため、'
           '集計後の xlsm が 307KB → 6,985KB(sheet4 29.8MB・sheet5 20.8MB を圧縮)。V-3 のテストコピー(9/16 06:16)も同じ 6,995KB だったが、検証項目に大きさが無く見落とした。'
           'データ・集計結果は正しい。修正案 V-4: 書式を付けるのを書き込む行数分だけにし、集計前に旧範囲の書式を General に戻す。本番反映(Q16)を V-4 の前にするか後にするかの判断が必要。',
}
# 英樹が判断できない前提の質問(ChatGPTへの確認シートにも載せる)
CHATGPT_ASK = set()   # Q17 は判断済み(CHATGPT_DECISIONS)
...
FINAL_CHECK_ACTIVE = False   # 8月分は 2026-09-21 に反映済み
...
def build():
    ...
    for r, (no, kind, text, target, opts, src) in enumerate(Q, 2):
        ...
        # ChatGPT が判断済みの質問は英樹の回答が無くても「ChatGPT判断済」(答え直しは不要。答えれば「回答済」になる)
        wq.cell(r, 9).value = f'=IF(F{r}<>"","回答済",IF(LEFT(K{r},7)="ChatGPT","ChatGPT判断済",IF(K{r}<>"","再確認","未回答")))'
    ...
    for r in range(2, wq.max_row + 1):
        ...
        if ans_v == 'ChatGPTに確認する' or (kind == APPROVE and ans_v == '不明') or str(no) in CHATGPT_DECISIONS or str(no) in CHATGPT_ASK:
            ...
            # 判断済みでも英樹が別の選択肢を答えていて、判断文がその選択肢を採用していなければ黄色(再判断が必要)
            dec = CHATGPT_DECISIONS.get(str(no)) or ''
            conflict = bool(dec) and ans_v not in (None, '', 'ChatGPTに確認する', '不明') and str(ans_v) not in dec
            wc.cell(rr, 6).fill = YELLOW if (not CHATGPT_DECISIONS.get(str(no)) or conflict) else GRAY
            for c in (2, 4, 5, 6):
    ...
    # 2026-09-21 回答16件を本番反映済み → 生成停止(回答済みの証跡は Output/Archive/2026-09/証跡/…回答済16件…xlsx)。
    # 黄色セルが減った後に再生成すると別の行が「最終確認」として出てしまうため、次の月次で使うときは FINAL_CHECK_ACTIVE を True にして対象月を見直す
    n_cost, n_cost_prev = (write_final_sheet(wb, previous_final_answers()) if FINAL_CHECK_ACTIVE else (0, 0))
