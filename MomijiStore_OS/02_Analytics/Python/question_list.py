...
TASKS = [
    ...
    # ChatGPT 2026-09-17: V4-x は今回限りの臨時作業(通常の在庫更新手順ではない)。英樹の操作は VBE の入替だけにし、
    # 読込→集計→保存は V-4 の自動実行(AutoImport/AutoAggregate)で Claude Code が行う。区分は docs/在庫更新_操作区分.md
    ('V4-1', '【今回限り】V-4 モジュールをコピーに入れる', f'{V4}/楽天在庫金額集計ツール_v1.0.xlsm(**コピー**。9/16 17:54 保存の 4.7MB のもの)', 'Excelで開く', 'マクロを有効にして開く(ボタンは押さない)', 'シートが見える', ''),
    ('V4-2', '', '同上', 'VBE', 'option＋F11 → 左で Module_Rakuten_Tool を選択 → メニューバー ファイル → 「Module_Rakuten_Tool の削除...」→ エクスポートは「いいえ」', '一覧から消える', ''),
    ('V4-3', '', '同上', 'VBE', 'ファイル → ファイルのインポート... → ' + f'{V4}/Module_Rakuten_Tool_V4.bas(9/18 版・自動実行つき)', 'Module_Rakuten_Tool が1つだけ戻る → VBEを閉じる', ''),
    ('V4-4', '', '同上', '保存', '⌘S → 閉じる(Excel に他のブックも開いていない状態にする)→ チャットで「V4 モジュール入替済」(**英樹の操作はここまで**)', 'Claude Code が 読込→集計→保存 を2回(8/3資料→9/16資料)自動で実行し verify_v4_copy.py 1/2 で判定。ダイアログが出て止まった場合だけ英樹に依頼', ''),
]
DONE = [
