...
TASKS = [
    ...
    ('T-3d-1', '最新在庫への更新④-1: 楽天CSVに無い94件をどうするか決める', LIST_NAME, 'シート「質問」の Q14',
     '新しい在庫CSV(9/16 08:11)に無い94件(在庫あり84件・数量469個・評価額約129万円。クーリア系が多い)を、在庫リストから 消す／残す(在庫0にする)／残す(そのまま) から選ぶ',
     'Q14 の「状態」が「回答済」になる', ''),
    ('T-3d-2', '最新在庫への更新④-2: 在庫ツールの**コピー**で取込と集計(2ボタン)', 'MomijiStore_OS/01_InventoryManagement/SourceData/V3_stock_test/楽天在庫金額集計ツール_v1.0.xlsm(**コピー**)',
     'ボタン', '**Claude Code が「着手OK」と伝えてから**(Q14の答えでImportを差し替えるため)。開く → 「楽天在庫ファイル読込」→ はい →(アクセス許可は「アクセス権を付与」)→ OK → 「集計実行」→ OK → ⌘S → 閉じる',
     'コピーの読込日時・集計日時が今になる → チャットで「コピーで集計した」', ''),
    ('T-3e', '最新在庫への更新⑤: 差分を見て本番へ進めるか決める', LIST_NAME, 'シート「質問」に追加される Q(差分提示後)',
     'Claude Code が差分(数量・評価額・共有在庫・原価未登録・基準日時)を出したら、実施する／しない を選ぶ', '本番の取込日時・集計日時が更新される(実施のとき)', ''),
]
DONE = [
    ...
    ('T-3a〜c', '最新在庫の資料取得', '楽天 dl-normal-item_20260916081129-1.csv(08:11・1ファイルで全部・b087lzdd59は訂正済、b0dktchdgkはRMS修正済で一覧へ反映)／Amazon inventoryList 2本(08:15)'),
...
def build_questions():
    ...
    Q.append(('Q14', APPROVE,
              '【判断・在庫リスト】2026-09-16 08:11 の楽天在庫CSV(1ファイルで全部)に、いまの在庫リストの94件が含まれていません(在庫あり84件・数量469個・評価額 約1,286,025円。クーリアのセット品・福袋など)。'
              'RMSで販売終了・削除した商品と考えられます。在庫リストからどうしますか？(消しても Archive に旧リストは残ります)',
              '楽天在庫リスト_import.xlsx の94行(差分CSV「CSVに無い」を参照)', '消す(RMSに合わせる)／残す(在庫0にする)／残す(そのまま)／ChatGPTに確認する', '最新在庫への更新 T-3d'))
