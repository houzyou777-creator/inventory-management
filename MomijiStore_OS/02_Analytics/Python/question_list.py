...
TASKS = [
    ...
    ('V4-1', 'V-4 コピー検証(1回目: 8/3資料 864行)', f'{V4}/楽天在庫金額集計ツール_v1.0.xlsm(**コピー**。本番と同一のものを 13:38 に複製)', 'Excelで開く', 'マクロを有効にして開く', 'シートが見える', ''),
    ('V4-2', '', '同上', 'VBE', 'option＋F11 → 左で Module_Rakuten_Tool を選択 → メニューバー ファイル → 「Module_Rakuten_Tool の削除...」→ エクスポートは「いいえ」', '一覧から消える', ''),
    ('V4-3', '', '同上', 'VBE', 'ファイル → ファイルのインポート... → ' + f'{V4}/Module_Rakuten_Tool_V4.bas', 'Module_Rakuten_Tool が1つだけ戻る → VBEを閉じる', ''),
    ('V4-4', '', '同上', 'ボタン', '「楽天在庫ファイル読込」→ 上書き「はい」→(アクセス許可が出たら「アクセス権を付与」)→ 完了「OK」', 'K列 読込日時が今(Import は 8/3資料のまま。これは意図どおり)', ''),
    ('V4-5', '', '同上', 'ボタン', '「集計実行」→ OK', '完了メッセージ: 総在庫金額 15,077,081 / 数量 6,801 / 未登録 20 のはず(違っても閉じてよい。Claude Code が調べる)', ''),
    ('V4-6', '', '同上', '保存', '⌘S → 閉じる → チャットで「V4 1回目終了」', 'Claude Code が verify_v4_copy.py 1 で判定(ファイルサイズ・識別子・空欄・集計値・数式・ボタン・VBA)し、Import を 9/16資料(814行)に差し替える', ''),
