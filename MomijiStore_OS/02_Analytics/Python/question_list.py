...
TASKS = [
    ...
    ('T-3e', '最新在庫への更新⑤: 本番へ進めるか決める', LIST_NAME, 'シート「質問」の Q16',
     'コピーの実測(数量5,911・評価額12,621,422・原価未登録59)は予測と完全一致。未確認項目(未掲載94件・原価未登録59件・Amazon基準日時不明・ファイル肥大 Q17)を読んで 実施する／しない／ChatGPTに確認する を選ぶ。'
     '本番の操作はまだしない(Q16の答えとChatGPTのレビューの後、Claude Code が「着手OK」を出してから)',
     'Q16 の「状態」が「回答済」になる', ''),
    ('T-3f', 'ChatGPTに見せて判断を貼る(2件)', LIST_NAME, 'シート「ChatGPTへの確認」',
     'シート「ChatGPTへの確認」の Q14(英樹「消す」と ChatGPT「別枠管理」が食い違い)と Q17(V-3の副作用: 集計後にファイルが 300KB→7MB)の行を ChatGPT に見せ、返ってきた判断を F列(黄色)に貼る。'
     'チャットに貼ってもよい(Claude Code が転記します)',
     'F列に ChatGPT の判断が入る', ''),
]
DONE = [
...
def build():
    ...
    for r in range(2, wq.max_row + 1):
        ...
        if ans_v == 'ChatGPTに確認する' or (kind == APPROVE and ans_v == '不明') or str(no) in CHATGPT_DECISIONS or str(no) in CHATGPT_ASK:
            ...
            # 判断済みでも英樹が別の選択肢を答えていれば黄色(再判断が必要)
            conflict = bool(CHATGPT_DECISIONS.get(str(no))) and ans_v not in (None, '', 'ChatGPTに確認する', '不明')
