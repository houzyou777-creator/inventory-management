...
TASKS = [
    ...
    ('T-3a', '最新在庫への更新①: RMSの商品番号 1件がまだ直っていない', 'RMS → 商品管理 → 管理番号 b0dktchdgk',
     '商品番号', '2026-09-16 08:11 の在庫CSVでは b087lzdd59 は `5036/1273` に直っていた✅。**b0dktchdgk(SKU b0dfyjnwjt)は `4987176260659-2` のまま**。正しい値 `4987176260635-2` に訂正する',
     'RMSの商品ページで `4987176260635-2` になっている → チャットで「直した」', ''),
    ('T-3b', '最新在庫への更新②: 在庫CSVの**続きのファイル**を置く', 'RMSのダウンロード結果(…-1.csv の続き)',
     'ダウンロード', '置いてもらった `dl-normal-item_20260916081129-1.csv` は**1つ目**です(RMSは大きな一覧を -1, -2… に分けます)。同じダウンロードの `…-2.csv`(あれば -3 も)を **Excelで開かず** '
     'MomijiStore_OS/01_InventoryManagement/import/Rakuten/ に置く。無ければ「1つだけだった」と教えてください(その場合 94件が楽天から消えたことになるので確認します)',
     'import/Rakuten に -2.csv がある(または「1つだけ」の連絡)', ''),
    ('T-3c', '最新在庫への更新③: Amazonの在庫ファイルの取得時刻を確認', 'MomijiStore_OS/01_InventoryManagement/import/Amazon/',
     '2ファイル', '`1296040_inventoryList_202608_2.csv`(581行)と `…FBA_202608_2.csv`(5行)を受け取りました(ファイル時刻 9/16 08:15)。名前に 202608 とあるので、**今日ダウンロードしたもの**か教えてください。今日のものならこのまま使います',
     'チャットで「今日落とした」or「8月のもの」', ''),
