...
TASKS = [
    ...
    ('V4-1', 'V-4 コピー検証 1回目のやり直し(8/3資料 864行)', f'{V4}/楽天在庫金額集計ツール_v1.0.xlsm(**コピー**。9/16 17:54 に保存した 4.7MB のもの。そのまま使う)', 'Excelで開く', 'マクロを有効にして開く', 'シートが見える', ''),
    ('V4-2', '', '同上', 'VBE', 'option＋F11 → 左で Module_Rakuten_Tool を選択 → メニューバー ファイル → 「Module_Rakuten_Tool の削除...」→ エクスポートは「いいえ」', '一覧から消える', ''),
    ('V4-3', '', '同上', 'VBE', 'ファイル → ファイルのインポート... → ' + f'{V4}/Module_Rakuten_Tool_V4.bas(**9/17 に修正した新しいもの**。同じ名前)', 'Module_Rakuten_Tool が1つだけ戻る → VBEを閉じる', ''),
    ('V4-4', '', '同上', 'ボタン', '「楽天在庫ファイル読込」→ 上書き「はい」→(アクセス許可が出たら「アクセス権を付与」)→ 完了「OK」', 'K列 読込日時が今(Import は 8/3資料のまま。これは意図どおり)', ''),
    ('V4-5', '', '同上', 'ボタン', '「集計実行」→ OK', '完了メッセージ: 総在庫金額 15,077,081 / 数量 6,801 / 未登録 20(前回と同じはず)', ''),
    ('V4-6', '', '同上', '保存', '⌘S → 閉じる → チャットで「V4 1回目やり直し終了」', 'Claude Code が verify_v4_copy.py 1 で判定(**ファイルが 4.7MB → 約300KB に戻ること**が今回の要点)。PASS なら Import を 9/16資料(814行)に差し替える', ''),
    ('V4-7', 'V-4 コピー検証(2回目: 9/16資料 814行・行数が減る)', '同上', 'ボタン', '**Claude Code が「2回目OK」と伝えてから**: 開く →「楽天在庫ファイル読込」→ はい → OK →「集計実行」→ OK', '完了メッセージ: 12,621,422 / 5,911 / 59 のはず', ''),
    ('V4-8', '', '同上', '保存', '⌘S → 閉じる → チャットで「V4 2回目終了」(**コピー検証の最終操作**)', 'Claude Code が verify_v4_copy.py 2 で判定(旧データが残らない・肥大再発なし)。PASS なら本番の操作(V-4差し替え＋最新在庫反映)をまとめて提示', ''),
]
DONE = [
