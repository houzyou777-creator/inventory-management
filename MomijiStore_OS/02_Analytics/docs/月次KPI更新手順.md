...
② 要対応一覧を確定 python3 build_todo_lists.py <月> <Transaction.csv>
      ↓          読み取り専用。修正する順番で6一覧を出力し、人が中身を確認する
③ register      python3 sync_cost_master.py register <月> / register-amazon <月>
      ↓          ★ユーザーの承認が必要。実行前に商品マスターのバックアップを取る
④ 一覧を再生成   ②をもう一度実行する
```

**この順番は固定である。** 入れ替えてはならない。

| なぜこの順番か | |
|---|---|
| ②が③より先 | 何を登録するのかを人が見てから登録する。`register` はマスター(正本)を書き換えるので、中身を確認せずに走らせない |
| ④が③の後 | 登録した商品は中身が空のまま入る。**再生成して初めて「次に何を直すか」が見える。**①新商品が0件になり、その分が③商品情報不足へ移る |
| ①が②より先 | 一覧はKPIシートの当月実績(出荷数・売上)で優先順位を決めるため、分析が終わっていないと順番が付かない |

④は「改善したか」を数字で確かめる場でもある。**再生成したときに件数が減っていることをもって、改善したと判断する。**

> ⚠️ **実行前に必ずレポートの列構成を検証すること**(2026-09-08 の事故を参照)。
> Amazonは予告なく列を変える。**位置ではなく見出し名で読む**のが唯一の防御である。
