...
| 項目 | 内容 |
|------|------|
| **実施頻度** | **月次(毎月月末)を基本**。季節商品・高回転商品は週次を推奨(RULE 05) |
| **開始条件** | 出荷作業が終了し(棚卸中は出荷・仕入を停止)、事前バックアップを取得し、棚卸入力シートが空である状態。実地カウントは**2名でダブルチェック**を推奨(RULE 05) |
| **終了条件** | 全対象行が「確定済」となり、在庫異動ログに全件記録され、**差異が大きい商品の原因調査が完了**し、**出品テーブルを更新した**状態(RULE 05 棚卸後チェックリスト) |
| **入力** | 棚卸入力シート(内部管理ID・保管区分・実棚数・棚卸担当者・棚卸日)、帳簿在庫 |
...
| **承認** | **要** — 差異の確定 |
| **例外処理** | ①**処理中にエラーが発生した場合はロールバックし、処理前の状態へ戻す**(`ConfirmInventory`にエラーハンドラとロールバックが実装済)②入力エラー・IDの不存在・保管区分の無効はエラー表示され、**在庫は変更されない**(`03_トラブル対応マニュアル`)③差異が大きい商品は差異理由欄へ原因を記入する |
| **⚠️ 照合で判明した重要事項** | **このプロセスは棚卸専用ではない。** RULE 04(在庫修正)と RULE 06(仕入反映)も同じ「棚卸入力シート → 棚卸確定」経路を使う。つまり**在庫を動かす唯一の正規経路がこのプロセスである**(在庫管理テーブルの直接書き換えは禁止)。プロセスの再定義が必要 — §5 Open Questions 5 |
...
### PRC-10 商品属性の候補提示・承認プロセス(2026-09-08 追加)

**`PRC-07` は「新商品を登録し、識別子を照合する」プロセスである。**
**本プロセスは「既存商品の属性を、AIの候補提示で埋める」— 対象も開始条件も異なる。**

```mermaid
graph LR
    A["出荷件数順に<br/>対象バッチを選ぶ"] --> B["AIが候補生成<br/>候補・根拠・確信度"]
    B --> C["候補ファイル出力<br/>Output/ ※正本ではない"]
    C --> D{"DEC-MST-05<br/>候補を採用するか"}
    D -- "承認 / 修正" --> E["商品マスターへ反映<br/>バックアップ→検証"]
    D -- "却下" --> F["Decision Logへ記録<br/>却下理由が次の材料"]
    D -- "保留" --> G["次バッチへ持ち越し<br/>記録しない"]
    E --> H["Decision Logへ記録<br/>DEC-MST-05"]
```

| 項目 | 内容 |
|------|------|
| **開始条件** | 商品マスターに **発送サイズ区分 または 商品重量が未入力**の商品が存在する |
| **終了条件** | 対象バッチの全商品が `承認` / `修正` / `却下` / `保留` のいずれかに決まり、承認分がマスターへ反映されている |
| **入力** | 商品マスター、出品テーブル、KPI管理シート(出荷実績)、外部情報(メーカー・Amazonカタログ) |
| **出力** | 候補ファイル(`Output/`)、商品マスターの2列、`intelligence.decisions` の記録 |
| **関連Business ID** | BIZ-MST-01, BIZ-MST-05 |
| **関連Decision ID** | **DEC-MST-05** |
| **Business Logic** | BL-2(正本の値は人が決める), BL-5(進捗表を別に持たない), BL-7(判断を記録する), BL-11(却下も記録する) |
| **担当** | **AI(候補生成)+ 人(承認)** |
| **AI関与** | **候補の生成と、根拠・確信度の提示まで。🚫 提示した値を正本へ書き込むことは不可** |
| **承認** | **要** — 1商品ごとに `承認 / 修正 / 却下 / 保留` |
| **例外処理** | **境界付近**(ネコポス/コンパクトの厚さ、重量が上限±15%)は根拠が強くても確信度を「低」とする。**外したときの損害が大きいため**。詳細設計は `Master_Design.md` §7 |

⚠️ **現時点の制約:** MCPサーバー停止中のため `intelligence.decisions` への記録ができない。
**承認済みの候補ファイル(Git管理下)を一次記録とし、復旧後に一括投入する**(`Master_Design.md` §7.8)。
...
| 指標 | 値 |
|------|-----|
| プロセス総数 | **10件**(2026-09-08 PRC-10 追加) |
...
| 4 | ~~在庫更新(PRC-04)と棚卸(PRC-05)の関係~~ | ✅ **判明**(2026-08-05 マニュアル照合)— 日常業務の全体フローは **仕入検品 → 在庫更新(棚卸入力)→ 棚卸確定 → 出品更新 → ログ確認 → バックアップ**(`06_運用フローチャート`)。PRC-04(モールCSV取込)とは別系統で、**現物起点の在庫変更はすべてPRC-05を通る** |
| 5 | **PRC-05の再定義が必要** | 棚卸・在庫修正(RULE 04)・仕入反映(RULE 06)が同一経路を共有していることが判明。**「棚卸プロセス」ではなく「在庫確定プロセス」として定義し直すべき。** 承認をいただければ改訂する |
