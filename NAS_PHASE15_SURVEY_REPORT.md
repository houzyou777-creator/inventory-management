...
作成日: 2026-08-05(v1.2 — SMB到達不可の経緯と解決を追記)
ステータス: **調査のみ実施(実装・設定変更は一切なし)**

> **⚠️ 本レポートの読み方:** §1・§2に記録された「SMB到達不可」は、**その時点の観測として正確**だったが、**現在は解決済み**である。記述は削除せず経緯として残す。時系列と原因は **§2.5「SMB接続の経緯」** を参照。
...
| **SMB接続不可の原因** | 🔬 調査時点では445/139が到達不可。SMBサービスまたはアクセス制御の設定による可能性が高いと記録(候補は§2)。**→ その後解決。現在はSMB接続正常(§2.5)** |
| Docker統合(最重要) | 📚 **可能。** UGOS ProはDocker + Docker Compose + Container Managerを標準搭載。AIスタックのCompose統合は現実的(§5) |
| Ollama | 📚 動作自体は可(CPU実行)。**現在のRAM 8GBでは3B級が上限**。増設後に再評価する(容量次第で7B〜14Bも視野) |
| ブロッカー | ~~SMB開通~~(解決済み §2.5)。残るはNASモデル名・RAM・ストレージ構成の確認(管理画面ログインが必要) |
...
**上記§2の観測は、その時点では正確だった。** 削除せず経緯として残す。以下は Decision Log(`Architecture.md`)の該当エントリと整合する。

| # | 段階 | 日時 | 観測・操作 | 結果 |
|---|------|------|-----------|------|
| 1 | **初回調査**(Phase1) | 2026-08-05 | NASのIP未提供のためLAN全体を走査。ARPテーブル記載の8台へポート確認 | SMB(445)応答は`192.168.0.1`(Atermルーター)のみ。NASを発見できず。**当初「NAS未検出」と誤って結論**(実際はARPに`192.168.0.8`が記録されていた) |
| 2 | **追加調査**(Phase1.5) | 2026-08-05 | ユーザーから固定IP `192.168.0.8` の提供を受け再調査 | ping応答0.6ms、80/443/9999/9443はOPEN、**445/139/22はclosed/filtered**。「SMBサービスまたはアクセス制御の設定により到達できていない可能性が高い/断定はできない」と記録(要因候補5つを併記) |
| 3 | **Finder確認**(ユーザー操作) | 2026-08-05 19:38 | ユーザーがFinderから `smb://192.168.0.8` へ接続 | **接続成功。** 共有`MomijiStore`を`/Volumes/MomijiStore`へマウント |
| 4 | **再調査**(読み取り専用) | 2026-08-05 | 前回と**同一コマンド・同一Mac・同一セッション**でポート再確認 | **445 OPEN・139 OPEN**(前回closed)、**22 closed・9999 OPEN**(前回から不変) |
| 5 | **原因の特定** | 2026-08-05 | 上記4の変化パターンを分析 | SMBに属する**445と139のみが同時に開き**、無関係な22/9999は不変。→ **NAS側のSMBサービスが調査時点では接続を受け付けていなかった。Mac側・Claude Code側の制約ではない** |
| 6 | **現在の状態** | 2026-08-05〜 | `mount` / `smbutil statshares -a` / `smbutil view` で検証 | **正常。** SMB 3.1.1・署名AES_128_GMAC有効・暗号化OFF。共有は`MomijiStore`(1.8TiB・使用2.0GiB・ほぼ空)と`IPC$`。サーバー名`MomijiStore-NAS`。**Finder / CLI / Claude Code が同一SMBセッションを共有できることを確認** |

**未確定事項:** SMBサービスの状態が「なぜ」変わったのか(管理画面での設定変更によるものか、NAS起動直後などでサービスが後から起動したのか)は、Mac側からは断定できない。**後者だった場合、SMBが停止している時間帯が存在することになり、Phase3の自動バックアップに監視が必要になる。**

**この経緯から得た運用上の教訓:**
1. **ポート応答の不在は「恒久的な遮断」を意味しない。** サービスの起動状態という時間的要因を必ず疑う
2. **SMBマウントが外れると `/Volumes/MomijiStore` はMac内蔵ディスク上の空フォルダになる。** この状態でrsync等が動くと内蔵ディスクへ書き込み続けるため、**バックアップ処理には目印ファイルによるマウント検証を必須とする**(Phase3で実装)
3. マウントはユーザーセッションに属する(`mounted by hide0726`・権限700)ため、バックグラウンドジョブからは見えない可能性がある → 永続マウント方式の決定が必要(§Phase2開始条件)
...
| ⑩ | Ollama | **動作可(CPU実行)。実機はDXP4800 GT / Ryzen Embedded R2514 / RAM 8GB。** GPU非搭載のためCPU実行となり、**現RAM 8GBでは3B級(llama3.2等)が上限**。増設後に7B以上を再評価する。なお常駐させず必要時のみ起動するジョブ型運用を前提とする |
...
### 実機構成(2026-08-05 ユーザー確認により確定)🔬

| 項目 | 確定値 |
|------|--------|
| 機種 | **UGREEN DXP4800 GT** |
| CPU | **AMD Ryzen Embedded R2514** |
| RAM | **8GB**(将来増設予定) |
| ストレージ | **M.2 SSD RAID1 約1.85TB**(共有`MomijiStore`の実測1.8TiBと一致) |
| HDD | 未搭載/未確定(DXP4800 GTは3.5インチベイを備える。本数・容量はInfrastructure実装時に確定) |
| SMB | 接続確認済(SMB 3.1.1) |
| Git | GitHub運用確認済 |

**この構成から言えること:** 現時点のNASストレージは**全てM.2 SSD**であり、当初想定した「HDD Volume + SSD Volume」の役割分担は現状では成立しない。Docker・DB・Git・バックアップはいずれもSSD上に置かれることになる。HDDを増設した場合に「SSD=実行系(Docker/DB)、HDD=保管系(バックアップ/アーカイブ/AIモデル)」へ再配置する設計とする。**RAM 8GBは常駐コンテナ数の制約になるため、常駐はPostgreSQL+MCPのみとする方針をより強く支持する。**

**⚠️ 以下§4の各項目は、上記確定前に作成した📚文献+💡設計提案である。** HDD/Volume詳細はInfrastructure実装時に再確定する。
