...
- **NAS上で直接コードを編集することを禁止する。** 修正は必ず **Git → Commit → GitHub → NASへDeploy** の順で行う
- **NASは「実行環境」であり、コードの保管場所ではない。** NASが停止しても、**Git Repositoryだけで完全復旧できる構成**を維持する
- NAS上のデータ(Excelスナップショット等)は**正本ではない**。正本はリポジトリ側にある

### GitHubへのPush基準

**Pushする:** 実装完了時 / 重要な節目 / STEP完了時
**Pushしない:** 一時的な検証コード / デバッグコード / 未完成コード
