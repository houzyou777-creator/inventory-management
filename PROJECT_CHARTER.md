...
ステータス: **v0.3 — 承認済み(2026-08-05)**
作成日: 2026-08-05(v0.3 — Design Principle「シンプルさを優先する」追加)
...
2. **段階移行** — 現段階(〜Phase3)は「実行はMac、資産はNAS」。最終ゴールはNAS上のDocker統合基盤(Claude Code / Git / AI / 在庫管理DB / バックアップ)で、Macは開発端末に徹する
3. **安全性 > 保守性 > 拡張性 > 速度** の優先順位
4. **AIが読み書きできる形式を優先する** — 属人的なファイルより構造化データ(将来的にExcel→DB移行)
5. **Googleスプレッドシート移行を見据える** — VBA依存最小化・数式優先・シート構造/列名/内部ID不変(詳細はCLAUDE.md)
6. **ハードコーディング禁止**、コメントはWHY中心
7. 単一障害点を作らない — 重要データは「Macローカル + GitHub + NAS」の3か所に存在させる
...
| **Phase1.5** | **NAS接続確認 + 開発環境調査(Docker/Git Server/MCP/DB/Ollama等13項目・調査のみ)** | 未着手(NAS固定IP待ち) |
| Phase2 | 共有フォルダ設計承認・作成・権限設定 | 未着手 |
